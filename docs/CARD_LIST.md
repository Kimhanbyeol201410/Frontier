...
| 광란의 열기 | 모든 적에게 피해 2회. (걸작 «멈출 수 없는 열기 +5» 변환 전용) |
...
| 모루의 기억 | 모든 적에게 [취약]·[약화] 부여 후 다타격. (걸작 «모루의 잔향 +5» 변환 전용) |
...
| 연마실 | 사용 시 [활력] 획득. |
| 열 교환 | 방어도. 손패 1장을 뽑을 더미 맨 위로 옮기고 강화. 열기 감소. |
...
| 접쇠 | 다음 강화 카드 1장 일정 횟수 재사용. 열기 임계 이상이면 +. |
