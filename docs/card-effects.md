...
| 단조 | `ForgingCard` | 2 | 공격 | Basic | 피해 **8** → 열기 **+10** → 손패에서 **자기 제외·강화 가능**한 카드 1장 강화 | 피해 **+3** (열기·강화 횟수는 동일) |
| 유냉 | `OilCoolingCard` | 1 | 스킬 | Basic | 방어도 **7** → 현재 열기와 **5** 중 작은 만큼 열기 감소 | 방어도 **+3**, 열기 감소량 **+5** → 최대 10 감소 |
